...
def flight_schedule(first=False) -> dict:
    ...
    with open('pilot_data.csv', 'r') as f:
        ...
        for i in reader:
            key = 'pilot' + i[0]
            schedule.update(
                {key: {'name': i[1], 'flight_id': i[2], 'coord': i[3], 'start_time': i[4], 'end_time': i[5]}})
    if first:
        print('Flight schedule loaded.....')
    else:
        for key1, value1 in schedule.items():
            print(
                f'Pilot {key1} name is {value1['name']} they are scheduled to fly on {value1['start_time']}')
    return schedule


def display_flight_time(schedule: dict):
    pilot_id = input(
        "Type the pilot id for the start date/time are you wanting to see?")
    if pilot_id in schedule.keys():
        print(
            f'{schedule[pilot_id]['name']} has a scheduled flight for {schedule[pilot_id]['start_time']}')
    else:
...
def check_in(pilot_id: str, schedule: dict, time_checked_in: str):
    ...
    if pilot_id in schedule.keys():
        converted_time = datetime.strptime(
            schedule[pilot_id]['start_time'], '%Y-%m-%d %H:%M:%S')
...
def detect_conflict(pilot1: str, pilot2: str, schedule: dict):
    ...
    if distance < 500 and start_time1 == start_time2:
        print(
            f'Conflict detected between pilot {pilot1}({flightId1}) and pilot {pilot2}({flightId2}) at {coord1},{coord2}. Distance: {distance:.2f} km')
    else:
